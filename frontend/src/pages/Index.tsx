...
const PMDeliverables = ({
  onStageClick,
}: {
  onStageClick: (deliverable: string, stage: string) => void;
}) => {
  const deliverables = [
    {
      name: "Piping Layout - Zone 1",
      stages: ["IDC", "IFR", "IFA", "AFC"],
      assigned: ["Anand", "", "", ""],
    },
    {
      name: "Electrical SLD - Main Panel",
      stages: ["IDC", "IFR", "IFA", "AFC"],
      assigned: ["Priya", "", "", ""],
    },
    {
      name: "Civil GA Drawing",
      stages: ["IDC", "IFR", "IFA", "AFC"],
      assigned: ["Rahul", "", "", ""],
    },
  ];
                ...
                {d.stages.map((stage, idx) => (
                  <td key={stage} className="px-4 py-3 text-center">
                    <Button
                      variant="outline"
                      className="w-24"
                      onClick={() => onStageClick(d.name, stage)}
                    >
                      {stage}
                      {d.assigned[idx] && (
...
const PMTimeline = () => (
  <div className="max-w-3xl mx-auto mt-8 text-xl text-blue-700">
  ...
  <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
    <div className="bg-white rounded-2xl shadow-xl p-8 w-full max-w-lg relative">
      ...
      <div className="text-slate-700 mb-4">
        [Panel for uploads, comments, time logging, status, and submit to docs]
      </div>
...
const PMProjects = () => (
  <div className="p-8 text-2xl text-blue-800">[Projects List Placeholder]</div>
);
const PMStageTracker = () => (
  <div className="p-8 text-2xl text-blue-800">[Stage Tracker Placeholder]</div>
);
const PMDocumentationSubmissions = () => (
  <div className="p-8 text-2xl text-blue-800">
    [Documentation Submissions Placeholder]
  </div>
);
const PMTeamCommunication = () => (
  <div className="p-8 text-2xl text-blue-800">
    [Team Communication Placeholder]
  </div>
);
const PMReportsLogs = () => (
  <div className="p-8 text-2xl text-blue-800">[Reports & Logs Placeholder]</div>
);
const PMSettings = () => (
...
const Index = () => {
  ...
  const handleRoleChange = (newRole: string) => {
    ...
    setActiveSection(
      roleViews[newRole as keyof typeof roleViews] || "projects"
    );
  };
  ...
  const renderMainContent = () => {
    ...
    switch (activeSection) {
      ...
      case "reports":
        if (currentUser.role === "PM Team") {
          return (
            <ProjectExecutionFlow section={activeSection} user={currentUser} />
          );
        }
        break;
      case "calendar":
  ...
  const [pmStagePanel, setPmStagePanel] = useState<{
    deliverable: string;
    stage: string;
  } | null>(null);
      ...
      <div className="min-h-screen bg-slate-50">
          ...
          <div className="flex items-center space-x-4 justify-end p-4">
            ...
            <Select value={currentUser.role} onValueChange={handleRoleChange}>
              ...
              <SelectContent>
                ...
                <SelectItem value="Estimation Department">
        ...
        <div className="flex">
          ...
          <main className="flex-1 ">
            ...
            {pmStagePanel && (
              <PMStagePanel
                deliverable={pmStagePanel.deliverable}
                stage={pmStagePanel.stage}
                onClose={() => setPmStagePanel(null)}
              />
            )}
          </main>
        </div>
